...
Usage:
    export ALBERT_API_KEY=your_api_key
    python tests/test_albert.py
"""
...
async def _test_memorize(service, file_path, output_data):
    ...
    print("\n[ALBERT] Test 1: Memorizing conversation...")
    memory = await service.memorize(
        resource_url=file_path, modality="conversation", user={"user_id": "albert_test_user"}
    )
...
async def _test_retrieve(service, queries, method, test_num, output_data):
    ...
    print(f"\n[ALBERT] Test {test_num}: {method.upper()}-based retrieval...")
    service.retrieve_config.method = method
    result = await service.retrieve(queries=queries, where={"user_id": "albert_test_user"})
...
async def test_albert_full_workflow():
    """Test OpenRouter integration with full MemU workflow."""
    if not api_key:
        pytest.skip("ALBERT_API_KEY environment variable not set")
    ...
    print("\n" + "=" * 60)
    print("[ALBERT] Starting full workflow test...")
    print("=" * 60)
    ...
    print("\n[ALBERT] Test 4: List memory items...")
    items_result = await service.list_memory_items(where={"user_id": "albert_test_user"})
    ...
    print("\n[ALBERT] Test 5: List memory categories...")
    cats_result = await service.list_memory_categories(where={"user_id": "albert_test_user"})
    ...
    output_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "examples", "output", "albert_test_output.json")
    )
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output_data, f, indent=2, default=str)
    print(f"\n[ALBERT] Output saved to: {output_file}")

    print("\n" + "=" * 60)
    print("[ALBERT] All tests completed!")
    print("=" * 60)


if __name__ == "__main__":
    asyncio.run(test_albert_full_workflow())
